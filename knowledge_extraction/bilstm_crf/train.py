...
if __name__ == '__main__':
    ...
    ndp = NerDataProcessor(max_len,vocab_size) # 调用data_helpers.py文件中处理数据的函数
    ...
    bilstm_crf = BiLstmCrfModel( # 调用bilstm_crf_model.py文件中构建模型的函数
            max_len,
            vocab_size,
            embedding_dim,
            lstm_units,
    ...
    reduce_lr = keras.callbacks.ReduceLROnPlateau( # 学习率衰减策略（回调）
        monitor='val_loss', 
        factor=0.5, 
        patience=4, 
        verbose=1)
    ...
    model.load_weights(bast_model_filepath) # 加载最佳权重
    model.save('./checkpoint_test/bilstm_crf_model.h5')

    pred = model.predict(test_X) # 预测测试集

    # 以下是评估 句子里面实体的准确率 召回率 f1值
